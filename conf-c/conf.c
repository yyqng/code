...
int argcheck(int argc,char **argv)
{
    ...
    {
        printf("Usage write config file: ./main w conf-file\n");
        printf("       read config file: ./main r conf-file\n");
        exit(-1);
    }
    return 0;
}
int cfgcreat( char *filename )
{
    if( !filename)
        return -1;
	...
	//初始化数据结构 出错则返回NULL
	if((conf=conf_creater_new( filename )) == NULL)
		return -1;

	//写配置(CONF_CREATER*, 键, 值, 注释) 成功时返回0，出错时conf_error可打印错误码信息 
...
int cfgread( char *filename )
{
    if( !filename)
        return -1;
	CONF *conf; 
	//打开并初始化数据结构,出错时返回NULL
	if((conf=conf_open( filename )) == NULL)
    {
        printf("conf_open error: %s\n", strerror(errno));
		return -1;
    }

	int code;
	//开始解析配置文件,成功返回0,出错时返回错误代码并打印错误信息
