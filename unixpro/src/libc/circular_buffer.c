...
int circular_buffer_timedwait_writeable(circular_buffer * cb, size_t const size, const struct timespec * timeout)
{
    assert(cb);
    ...
    while (writable_size(header) >= size) {
        ...
        // 阻塞在信号量上
        int const ret = sem_timedwait(&header->wsema, &tm);
...
int circular_buffer_timedwait_readable(circular_buffer * cb, size_t const size, const struct timespec * timeout)
{
    assert(cb);
    assert(timeout);
    ...
    while (readable_size(header) < size) {
        ...
        // 阻塞在信号量上
        int const ret = sem_timedwait(&header->rsema, &tm);
