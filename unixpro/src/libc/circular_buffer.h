 ...
 *
 * @return 返回0表示至少有size字节的数据可读；
 *         返回-1表示失败，超进返回后errno将被设置为ETIMEDOUT
 */
int circular_buffer_timedwait_readable(circular_buffer * cb, size_t size, const struct timespec * timeout);
 ...
 *         返回-1表示失败，超进返回后errno将被设置为ETIMEDOUT
 */
int circular_buffer_timedwait_writeable(circular_buffer * cb, size_t size, const struct timespec * timeout);
