...
const char* TbBlogDAO::m_pCreateSql[s_createCount] = {TB_BLOG_CREATE_TABLE_SQL};
...
int TbBlogDAO::Init(DataBaseHandler *pDbHandler)
{
    if (pDbHandler == NULL)
    {
        return -1;
    ...
    // table exist sql
    if (m_pDbHandler->TableExist("TB_BLOG") == false)
    {// table create sql
    ...
    // insert sql
    sprintf(sqlBuffer, TB_BLOG_INSERT_SQL);
    ...
    // select sql
    sprintf(sqlBuffer, TB_BLOG_SELECT_PRODUCTID_SQL);
...
int TbBlogDAO::Insert(const SPYDATA_BLOG &data)
{
	char targettype[8] = "";
    char acctype[8] = "";
    ...
    try
    {
        ...
        m_pInsert->SetString(13, data.BlogId);

		if (0 != data.CreateTime)
	        m_pInsert->SetInt(14, data.CreateTime);
		else
			m_pInsert->SetIntNull(14);
		
        m_pInsert->SetString(15, data.Longitude);		
        m_pInsert->SetString(16, data.Latitude);
        m_pInsert->SetString(17, data.CoordinateType);
        m_pInsert->SetString(18, data.Title);
		
		if (0 != data.ViewTimes)
	        m_pInsert->SetInt(19, data.ViewTimes);
		else
			m_pInsert->SetIntNull(19);
...
int TbBlogDAO::QueryByProductId(const char *account, std::vector<SPYDATA_BLOG> &vecData)
{
    int count = 0;
    ...
    try
    {
        ...
        {
            SPYDATA_BLOG data;
			pRs->GetString(2, data.Common.TargetId);
			...
			pRs->GetString(14, data.BlogId);
			pRs->GetInt(15, CreateTime);
			pRs->GetString(16, data.Longitude);
			pRs->GetString(17, data.Latitude);
			pRs->GetString(18, data.CoordinateType);
			pRs->GetString(19, data.Title);
			pRs->GetInt(20, data.ViewTimes);
